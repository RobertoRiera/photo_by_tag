...
__author__ = 'pedro'  # adaptación a pyton del código de Modesto
...
class Normalization:
    ...
    def normalize_gray_img(self, grey_img, lex, ley, rex, rey, kind=Kind_wraping.HS):
        ...
        c_wraping_img = self.__obtain_wraping_by_kind(kind)
        gray_img_norm = c_wraping_img.normalize_by_eyes(gray_img_rot, oixr, oiyr, odxr, odyr)
    ...
    def __obtain_wraping_by_kind(self, kind):
        c_wraping_img = None
        if kind == 'HS':
...
class Utils4Normalization:
    ...
    def add_padding(grey_img, lex, ley, rex, rey):
        ...
        img_padding = np.zeros(shape=(int(isy + y_padding * 2), int(isx + x_padding * 2)))
